...
% underline the fact that the E value IS QMC.
...
\end{equation}
\subsubsection{Constructing the payoff}
The payoff of a european call option at maturity is defined as 
\begin{equation}\label{eq:callPayoff}
  f(S_T) = \max \{ 0, S_T - K \},
\end{equation}
with $K$ the strike price and $T$ the maturity date. Numerically the payoff (and any other function) can be realized using piecewise linear functions for $S_T \le K$ and $S_T geq K$ which are dependent on the given stock price $S_i$. On a quantum computer we are interested in an operator
\begin{equation}\label{eq:payOffOperator}
  \ket{i}_n \ket{0} \rightarrow \ket{i}_n (\text{cos}[f(i)] \ket{0} + \text{sin}[f(i)\ket{1}]),
\end{equation}
where $f:\{ 0,...,2^n-1 \} \rigtharrow [0,1]$. $f$ is on a quantum computer dependent on the stock price encoded in a quantum state and the piecewise linear functions are implemented usind controlled Y-gates.\\
